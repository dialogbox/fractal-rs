...
class Te {
  ...
  constructor(e, t, n, r, _, o, a, f, b, s, l, g, I, w, ie, fe, ue, se, k) {
    const W = i.renderparams_new(e, t, n, r, _, o, a, f, b, s, l, g, I, w, ie, fe, ue, se, k);
...
function xt() {
  return { __proto__: null, "./fractal_rs_bg.js": { __proto__: null, __wbg_Window_cf5b693340a7c469: function(e) {
    ...
    const n = Ye(t), r = O(n, i.__wbindgen_malloc, i.__wbindgen_realloc), _ = L;
    v().setInt32(e + 4, _, true), v().setInt32(e + 0, r, true);
    ...
    v().setFloat64(e + 8, d(r) ? 0 : r, true), v().setInt32(e + 0, !d(r), true);
    ...
    v().setInt32(e + 4, o, true), v().setInt32(e + 0, _, true);
...
function Dt(c, e) {
  return c = c >>> 0, Ft().subarray(c / 1, c / 1 + e);
}
function Ct(c, e) {
...
function O(c, e, t) {
  if (t === void 0) {
    ...
    return Ie().subarray(f, f + a.length).set(a), L = a.length, f;
  }
...
async function Vt() {
  ...
  let f = 800, b = 600, s = 800, l = 600, g = h.fromNumber(-1), I = h.fromNumber(0), w = h.fromNumber(3), ie = "#9cacba", fe = "#1049ac", ue = 0, se = 0.8;
  const k = /* @__PURE__ */ new Set();
  let W = 0, $ = 0, J = 0;
  const _e = 0.05, Ee = 0.85, bt = 0.02, dt = 0.02, $e = (u) => {
    const p = parseInt(u.slice(1, 3), 16) / 255, B = parseInt(u.slice(3, 5), 16) / 255, P = parseInt(u.slice(5, 7), 16) / 255;
    return [p, B, P];
  }, Le = new URLSearchParams(window.location.search), je = Le.get("x"), Ze = Le.get("y"), He = Le.get("w");
  if (je) try {
    g = h.fromString(je);
  } catch (u) {
...
w=${w.toString()}`;
    ...
    u.searchParams.set("x", g.toString()), u.searchParams.set("y", I.toString()), u.searchParams.set("w", w.toString()), Qe(u.toString());
  };
  const S = document.createElement("div");
  S.style.position = "relative", S.style.display = "flex", S.style.justifyContent = "center", S.style.alignItems = "center", S.style.boxShadow = "0 0 50px black";
  ...
  `, document.head.appendChild(Ke), S.style.transition = "transform 0.2s, border-radius 0.2s", document.body.appendChild(S), S.style.display = "grid";
  const Fe = (u) => {
    u.style.gridArea = "1 / 1", u.style.width = "100%", u.style.height = "100%", u.style.display = "block";
  };
  c.parentNode && c.parentNode.removeChild(c), S.appendChild(c), Fe(c), S.appendChild(V), Fe(V);
  const ce = document.createElement("canvas");
  ce.style.pointerEvents = "none", ce.style.zIndex = "50", S.appendChild(ce), Fe(ce);
  const Re = ce.getContext("2d"), me = () => {
    let u = false;
    const p = s > 0 && l > 0 ? s / l : 1, B = 3 * Math.max(1, p), P = h.fromNumber(B), T = (s || 800) * Number.EPSILON * 2, y = h.fromNumber(T);
    let C = false, E = false;
    w.lt(y) && (w = y, u = true, C = true), w.toNumber() <= T * 1.01 && (C = true), w.gt(P) && (w = P, u = true, E = true), w.toNumber() >= B * 0.99 && (E = true);
    const q = w.div(p), G = h.fromNumber(n), X = h.fromNumber(r), N = h.fromNumber(_), U = h.fromNumber(o), Y = G.add(w.div(2)), D = X.sub(w.div(2)), te = N.add(q.div(2)), ne = U.sub(q.div(2));
    let F = g, R = I;
    return Y.gt(D) ? F = G.add(X).div(2) : (F.lt(Y) && (F = Y), F.gt(D) && (F = D)), te.gt(ne) ? R = N.add(U).div(2) : (R.lt(te) && (R = te), R.gt(ne) && (R = ne)), F !== g && (g = F, u = true), R !== I && (I = R, u = true), { changed: u, atMin: C, atMax: E };
  }, Je = () => {
    f = window.innerWidth, b = window.innerHeight, S.style.width = "100vw", S.style.height = "100vh", S.style.margin = "0", document.body.style.margin = "0", document.body.style.overflow = "hidden", s = Math.floor(f * a), l = Math.floor(b * a), [c, V, ce].forEach((u) => {
  ...
  const et = (u, p) => {
    ...
    let y = t.querySelector("#info-text"), C = t.querySelector("#btn-container");
    y || (y = document.createElement("div"), y.id = "info-text", C ? t.insertBefore(y, C) : t.appendChild(y)), y.innerHTML = `
    ...
    `;
    const E = y.querySelector("#coords-detail");
    t.onmouseenter = () => {
      C && (C.style.display = "flex"), E && (E.style.display = "block");
    }, t.onmouseleave = () => {
      C && (C.style.display = "none"), E && (E.style.display = "none");
    };
    const q = s > 0 && l > 0 ? s / l : 1, G = w.div(q), X = g.sub(w.div(2)), N = g.add(w.div(2)), U = I.sub(G.div(2)), Y = I.add(G.div(2));
    if (oe) {
      V.width !== s && (V.width = s, V.height = l);
      let D = Math.floor(Math.min(s, l) / 2);
      D = Math.min(D, 64), D = Math.pow(2, Math.floor(Math.log2(D))), D < 1 && (D = 1);
      const te = u < D, [ne, F, R] = $e(ie), [We, Ge, Ne] = $e(fe), [Ue, Ve] = X.split(), [ze, qe] = N.split(), [at, gt] = U.split(), [lt, mt] = Y.split(), wt = new Te(Ue, Ve, ze, qe, at, gt, lt, mt, P, u, te, ne, F, R, We, Ge, Ne, ue, se);
      oe.render(wt);
    }
    if (u > 1) {
      let D = Math.floor(u / 2);
      D < 1 && (D = 1), requestAnimationFrame(() => et(D, p));
    }
  };
  let Me = null;
  const tt = (u) => {
    S.classList.remove("limit-barrel", "limit-pincushion"), S.offsetWidth, S.classList.add(u), Me && clearTimeout(Me), Me = window.setTimeout(() => {
      S.classList.remove("limit-barrel", "limit-pincushion"), Me = null;
    }, 300);
  }, z = () => {
    Oe++;
    const u = Oe;
    let p = Math.floor(Math.min(s, l) / 2);
    p = Math.min(p, 64), p = Math.pow(2, Math.floor(Math.log2(p))), p < 1 && (p = 1), requestAnimationFrame(() => et(p, u));
  };
  let ke = false, nt = 0, rt = 0, _t = h.fromNumber(0), ot = h.fromNumber(0);
  S.addEventListener("mousedown", (u) => {
    ke = true;
    const p = S.getBoundingClientRect();
    nt = u.clientX - p.left, rt = u.clientY - p.top, _t = g, ot = I;
  }), window.addEventListener("mousemove", (u) => {
    if (!ke) return;
    const p = S.getBoundingClientRect(), B = u.clientX - p.left, P = u.clientY - p.top, T = B - nt, y = P - rt, C = s / l, E = w.toNumber() / f, q = w.toNumber() / C / b, G = h.fromNumber(T * E), X = h.fromNumber(y * q), N = g, U = I;
    g = _t.sub(G), I = ot.sub(X), me(), (g.toString() !== N.toString() || I.toString() !== U.toString()) && z();
  }), window.addEventListener("mouseup", () => {
    ke = false;
  }), S.addEventListener("wheel", (u) => {
    u.preventDefault();
    const p = w, B = g, P = I, T = u.deltaY < 0 ? 0.9 : 1.1;
    let y = w.scale(T);
    const C = s / l, E = 3 * Math.max(1, C), q = h.fromNumber(E), G = (s || 800) * Number.EPSILON * 2, X = h.fromNumber(G);
    let N = false, U = false;
    y.lt(X) && (y = X, N = true), y.toNumber() <= G * 1.01 && (N = true), y.gt(q) && (y = q, U = true), y.toNumber() >= E * 0.99 && (U = true), T < 1 && N && tt("limit-pincushion"), T > 1 && U && tt("limit-barrel");
    const Y = S.getBoundingClientRect(), D = u.clientX - Y.left, te = u.clientY - Y.top, ne = D / f, F = te / b, R = w.div(C), We = y.div(C), Ge = w.sub(y), Ne = R.sub(We), Ue = Ge.mul(ne - 0.5), Ve = Ne.mul(F - 0.5), ze = g.add(Ue), qe = I.add(Ve);
    g = ze, I = qe, w = y, me(), (!w.eq(p) || !g.eq(B) || !I.eq(P)) && z();
  ...
  const ct = () => {
    const u = w, p = g, B = I;
    let P = false;
    if (k.has("ArrowUp") && ($ -= _e), k.has("ArrowDown") && ($ += _e), k.has("ArrowLeft") && (W -= _e), k.has("ArrowRight") && (W += _e), k.has("PageUp") && (J -= _e), k.has("PageDown") && (J += _e), W *= Ee, $ *= Ee, J *= Ee, Math.abs(W) < 1e-3 && (W = 0), Math.abs($) < 1e-3 && ($ = 0), Math.abs(J) < 1e-3 && (J = 0), W !== 0 || $ !== 0) {
      const T = s / l, y = w.toNumber() * bt;
      S.classList.contains("limit-pincushion") ? (W = 0, $ = 0) : (g = g.add(h.fromNumber(W * y)), I = I.add(h.fromNumber($ * y / T)), P = true);
    }
    if (J !== 0) {
      const T = 1 + J * dt;
      w = w.scale(T), P = true;
    }
    P && (me(), (!w.eq(u) || !g.eq(p) || !I.eq(B)) && z()), requestAnimationFrame(ct);
  };
  ct(), Je();
}
Vt();
